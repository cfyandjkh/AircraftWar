...
public class MusicThread extends Thread {
    ...
    public MusicThread(String filename,boolean repeat,int type ) {
        //初始化filename
        ...
        try {
            ...
            if(!repeat){
                ...
                if(type==BACKGROUND_MUSIC){
                    while (numBytesRead != -1&&!Game.getGameOverFlag()) {
                        //从音频流读取指定的最大数量的数据字节，并将其放入缓冲区中
                        numBytesRead =
                                source.read(buffer, 0, buffer.length);
                        //通过此源数据行将数据写入混频器
                    ...
                    while (numBytesRead != -1&&Game.getBossAliveFlag()&&!Game.getGameOverFlag()) {
                        //从音频流读取指定的最大数量的数据字节，并将其放入缓冲区中
                        numBytesRead =
                                source.read(buffer, 0, buffer.length);
                        //通过此源数据行将数据写入混频器
    ...
    public void run() {
        ...
        if(repeat){
            if(type==BACKGROUND_MUSIC){
                ...
                while (Game.getBossAliveFlag()&&!Game.getGameOverFlag()) {
                    play(stream);
                }
            }
        }
